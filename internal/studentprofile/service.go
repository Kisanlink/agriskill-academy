...
func (s *studentProfileService) GetProfile(userID string) (*StudentProfile, error) {
	fmt.Printf("🔍 DEBUG: Service.GetProfile called for userID: %s\n", userID)
	profile, err := s.repo.GetByUserID(userID)
	if err != nil {
		fmt.Printf("❌ DEBUG: Service.GetProfile error: %v\n", err)
		return nil, err
	}
	fmt.Printf("✅ DEBUG: Service.GetProfile success: %+v\n", profile)
	return profile, err
}
...
func (s *studentProfileService) UpdateProfile(profile *StudentProfile) error {
	...
	fmt.Printf("✅ DEBUG: Service.UpdateProfile completed successfully\n")
	return nil
}
...
func (s *studentProfileService) CreateProfile(profile *StudentProfile) error {
	...
	fmt.Printf("✅ DEBUG: Service.CreateProfile completed successfully\n")
	return nil
}

func (s *studentProfileService) AddCertificate(cert *Certificate) error {
	fmt.Printf("🔍 DEBUG: Service.AddCertificate called - StudentProfileID: %s, Name: %s, FileSize: %d bytes, FileType: %s\n", cert.StudentProfileID, cert.Name, cert.FileSize, cert.FileType)
	err := s.repo.AddCertificate(cert)
	if err != nil {
		fmt.Printf("❌ DEBUG: Service.AddCertificate error: %v\n", err)
		return err
	}
	fmt.Printf("✅ DEBUG: Service.AddCertificate completed successfully\n")
	return nil
}

func (s *studentProfileService) DeleteCertificate(certID string, userID string) error {
	fmt.Printf("🔍 DEBUG: Service.DeleteCertificate called - CertID: %s, UserID: %s\n", certID, userID)
	err := s.repo.DeleteCertificate(certID, userID)
	if err != nil {
		fmt.Printf("❌ DEBUG: Service.DeleteCertificate error: %v\n", err)
		return err
	}
	fmt.Printf("✅ DEBUG: Service.DeleteCertificate completed successfully\n")
	return nil
}
